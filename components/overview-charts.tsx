...
export function OverviewCharts({clientes, eventos}:{clientes:Cliente[]; eventos:Evento[]}) {
  const days = Array.from({length:7},(_,i)=>{
    const d=new Date(`${hoyAR()}T12:00:00Z`); d.setUTCDate(d.getUTCDate()-(6-i)); const key=d.toISOString().slice(0,10);
    return {name:d.toLocaleDateString("es-AR",{weekday:"short",timeZone:"UTC"}), ingresos:clientes.filter(c=>c.estadoPago==="PAGADO"&&isoDate(c.fechaCompra)===key).reduce((s,c)=>s+c.monto,0), eventos:eventos.filter(e=>isoDate(e.fecha)===key).length};
  });
